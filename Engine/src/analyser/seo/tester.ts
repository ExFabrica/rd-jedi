...
const $attributes = ($: any, search: string) => {
  const arr = [];
...
export const seoTester = function ({
  rules = [],
  ...
  let currentUrl = "toto.com";
  let currentRule = JSON.parse(JSON.stringify(emptyRule));
  let count = 0;
  ...
  const logMetaDescription = (meta) => {
    if (metaDescriptions.has(meta)) {
      siteResults.duplicateMetaDescriptions.push([metaDescriptions.get(meta), currentUrl]);
    } else {
      metaDescriptions.set(meta, currentUrl);
    }
  };

  const logTitleTag = (title) => {
    if (titleTags.has(title)) {
      siteResults.duplicateTitles.push([titleTags.get(title), currentUrl]);
    } else {
      titleTags.set(title, currentUrl);
    }
  };
  ...
  const runTest = (defaultPriority = 50, arrName) => {
    return (t, ...params) => {
      ...
      try {
        ...
        currentRule[arrName].push({ message: e.message, priority });
        return e;
      }
    };
  };
  ...
  const startRule = ({ validator, test, testData, ...payload }) => {
    if (currentRule.errors.length > 0)
      throw Error(
        "Starting a new rule when there are errors that haven't been added to results. Did you run 'finishRule'? ",
      );
    if (currentRule.warnings.length > 0)
      throw Error(
        "Starting a new rule when there are warnings that haven't been added to results. Did you run 'finishRule'? ",
      );
    currentRule = Object.assign(currentRule, payload);
  };
  const finishRule = () => {
    if (currentRule.errors.length === 0 && currentRule.warnings.length === 0) currentRule.success = true;
    results.push(currentRule);
    currentRule = JSON.parse(JSON.stringify(emptyRule));
  };
    ...
    try {
      ...
      if (siteWide) {
        ...
        result.aTags
          ...
          .filter((a) => {
            if (currentUrl !== '/') {
              return !a.href.endsWith(currentUrl);
            }
            return true;
          })
          .filter((a) => a.href !== currentUrl)
          .map((a) => a.href)
          .forEach((a) => internalLinks.push([a, currentUrl]));
      }
  ...
  const analyse = async (folder) => {
    ...
    const outResults = Object.keys(siteResults).reduce(
      ...
      console.log("outResults", outResults)
