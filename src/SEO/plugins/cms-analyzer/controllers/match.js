 ...
 */
  async find(ctx) {
    ...
    return entities.map(entity => sanitizeEntity(entity, { model: strapi.plugins['cms-analyzer'].models.match }));
  },
  async findOne(ctx) {
    const { id } = ctx.params;

    const entity = await strapi.plugins['cms-analyzer'].services.match.findOne({ id });
    return sanitizeEntity(entity, { model: strapi.plugins['cms-analyzer'].models.match });
  },
  async count(ctx) {
    if (ctx.query._q) {
